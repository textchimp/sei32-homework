...
let numOfStops = 0; // counter variable needs to exist outside of the object to return value
let numOfStops2 = 0;
 ...
 const mta = {
    ...
    stopIndex: function(line, end){
      return this.lines[line].indexOf(end);
    },
   ...
   stopsMadeToUnion: function(line, start) {
     // start --> union
     const stations = this.lines[line];

     //let numOfStops = 0;

     if ( this.startIndex(line, start) < this.stopIndex(line, 'Union Square') ) {
       for (let i = this.startIndex(line, start)+1; i <= this.stopIndex(line, 'Union Square'); i++) {
         console.log(stations[i]);
         numOfStops += 1;
       } // end of loop
     } else {
   ...
   },// end of stopsMadeToUnion
   stopsMadeAfterUnion: function(line2, end) {
     // start --> union
     const stations = this.lines[line2];

     //let numOfStops = 0;

     if ( this.startIndex(line2, 'Union Square') < this.stopIndex(line2, end) ) {
       for (let i = this.startIndex(line2, 'Union Square')+1; i <= this.stopIndex(line2, end); i++) {
         console.log(stations[i]);
         numOfStops2 += 1;
       } // end of loop
     } else {
   ...
   planTrip: function(line, start, line2, end){
     if (line === line2) {
       ...
       this.stopsMadeToUnion(line, start);
       console.log('Change at Union Square');
       this.stopsMadeAfterUnion(line2, end);
       console.log(numOfStops + numOfStops2, `stops in total`);
